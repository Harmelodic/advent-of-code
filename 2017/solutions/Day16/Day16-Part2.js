...
const originalLineUp = programs.split("");
...
fs.readFile("./Day16-Input.txt", (err, data) => {
    ...
    for (let i = 0; i < 1000000000; i++) {
        ...
        if (lineOfPrograms.toString() === originalLineUp.toString()) {
            break;
        }
    }

    let numberToActuallyRun = 1000000000 % cycles;

    for (let i = 0; i < numberToActuallyRun; i++) {
        data.toString().split(",").forEach(move => {
            switch (move[0]) {
                case "s":
                    let numberToMove = parseInt(move.substr(1));
