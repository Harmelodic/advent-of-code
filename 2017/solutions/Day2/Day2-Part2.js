...
var myFormattedInput = [] 

for (let i = 0; i < myRawInput.length; i++) {
    const rawRow = myRawInput[i];

    var formattedRow = rawRow.split("\t").map(number => parseInt(number));
    myFormattedInput[i] = formattedRow;
}

var solution = 0;

myFormattedInput.forEach(row => {
    row.forEach(loop1Element => {
        row.forEach(loop2Element => {
            if (loop1Element != loop2Element) {
                if (loop1Element % loop2Element == 0) {
                    solution += loop1Element / loop2Element;
                }
                else if (loop2Element % loop1Element == 0) {
                    solution += loop2Element / loop1Element;
                }
            }
        });
    });
...
// Holy hell, this is filthy ^^, but it works! - *cries in the corner*
